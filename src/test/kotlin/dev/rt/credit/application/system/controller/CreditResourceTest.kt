...
import dev.rt.credit.application.system.controller.CreditResourceHelper.creditDto
import dev.rt.credit.application.system.controller.CreditResourceHelper.customer
import dev.rt.credit.application.system.controller.CreditResourceHelper.invalidCreditDto
import dev.rt.credit.application.system.repository.CreditRepository
...
class CreditResourceTest {
  ...
  fun `when searching for a specific credit, then return 200 status`() {
    ...
    val creditCode = JsonPath.parse(responseBody)
      .read<String>("$.creditCode")
  ...
  fun `when searching for a credit with an invalid code, then return 400 status`() {
    ...
    mockMvc.perform(
      ...
      .andExpect(jsonPath("$.details.null")
        .value("Creditcode 42de0e2d-e158-457b-ad3d-97875b36141b not found"))
  }
